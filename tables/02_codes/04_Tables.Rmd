...
table2_2020_2021        <-      rbind(men_full_2020 %>%   dplyr::select(reg, prov, year, week, sex, group_age, n, E, low, upp), 
                                ...
                                group_by(year, group_age) %>% summarise(MoMo_Excess_Deaths_N = round(sum(n) - sum(E)), COVID_19_Deaths_N = sum(n), 
                                                                        Ratio_Momo_COVID_19_Deaths  = round((sum(n) - sum(E))/sum(n),2)) %>% 
                                                              bind_rows(summarise(., across(where(is.numeric), sum),
                                                              ...
                                                              mutate(Age_Group = case_when(group_age ==  "40_less" ~ "0-40", 
                                                                                           ...
                                                                                           TRUE ~ group_age), Period = year) %>% ungroup %>% 
...
```{r}
skim <- skim_with(numeric = sfl(max = ~ max(., na.rm = TRUE), min = ~ min(., na.rm = TRUE), median = ~ median(., na.rm = TRUE)), append = TRUE)
...
table3 <- table3_2020_2021 %>% dplyr::select(n, pr_edu_inf_25,pr_no_bl_mz, pr_insu, 
                               ...
                               skim() %>% 
                               dplyr::select(-c(numeric.hist, skim_type,n_missing, complete_rate, numeric.p0)) %>% 
                               relocate(numeric.median, numeric.sd, numeric.min, numeric.max, .after = numeric.mean) %>% 
                               rename(Variable = skim_variable, Mean = numeric.mean, Standard_deviation = numeric.sd,
                                      Percentile_25th = numeric.p25, Percentile_50th = numeric.p50,
                                      Percentile_75th = numeric.p75, Percentile_100th = numeric.p100,
                                      Maximum = numeric.max, Minimum = numeric.min, Median = numeric.median) %>%
                               mutate_each(funs(round(.,2)),2:10)
