...
library(xlsx)
```
C:/Users/braul/Desktop/02_projects/01_GITHUB/Excess_Mort
...
figures <- paste(main_directory, sep="", "/figures")
tables  <- paste(main_directory, sep="", "/01_Descriptive_analysis/tables/02_output")
moran   <- paste(main_directory, sep="", "/test_moran_global")
```
...
```{r}
men_2020_21_table1  <-   men_full_2020_21  %>% group_by(reg) %>% dplyr::summarise(Total_men  = sum(n), 
                                                                        E_men      = round(sum(E)), 
                                                                        Excess_men = round(sum(n) - sum(E)), 
                                                                        Upp_men    = round(sum(n) - sum(low)), 
                                                                        Low_men    = round(sum(n) - sum(upp))) %>% 
                                                                 bind_rows(dplyr::summarise(., across(where(is.numeric), sum),
                                                                                     across(where(is.character), ~"PERÚ"))) %>%
                                                                 dplyr::mutate(Excess_men1     = paste(round(Excess_men)," (",round(Low_men), " ","to",
                                                                                             " ", round(Upp_men),")", sep =""), 
                                                                        Excess_men_perc = round((Total_men-E_men)/E_men,3), 
                                                                        Excess_rate_men = round((Total_men-E_men)/Total_men,3) ) 


women_2020_21_table1  <- women_full_2020_21 %>% group_by(reg) %>% dplyr::summarise(Total_women  = sum(n), 
                                                                      E_women      = round(sum(E)), 
...
table1_2020_21_v1    <- both_2020_21_table1 %>% dplyr::select(-c(Excess, Low, Upp)) %>% 
                        ...
                        dplyr::rename(Abs_excess = Excess1, Abs_excess_men = Excess_men1, Abs_excess_women = Excess_women1, Relative_excess = Excess_perc,   
...
table1_2020_21_v2    <- both_2020_21_table1 %>% dplyr::select(-c(Excess1)) %>% 
                        ...
                        dplyr::rename(Abs_excess = Excess, Abs_excess_men = Excess_men, Abs_excess_women = Excess_women, Relative_excess = Excess_perc,   
                               Relative_excess_men  = Excess_men_perc, Relative_excess_women = Excess_women_perc )
...
table2_2020_2021        <-      rbind(men_full_2020_21   %>%  dplyr::select(reg, prov, year, week, sex, group_age, n, E, low, upp), 
                                ...
                                group_by(group_age, sex) %>%  dplyr::summarise(Excess_Deaths = round(sum(n) - sum(E)), 
                                                                             N = sum(n),
                                                                             E = round(sum(E))) %>% 
                                                                   bind_rows(dplyr::summarise(., across(where(is.numeric), sum),
                                                                   ...
                                                                   ungroup %>% 
                                                                   dplyr::select(Sex, Age_Group, Excess_Deaths, E, N,
                                                                                 Excess_Deaths_E, Excess_Deaths_N) %>% 
                                                                   arrange(Age_Group, Sex)
``` 

```{r}
a <-  women_full_2020_21 %>% mutate (pr_obes = as.numeric(NA), pr_tos = as.numeric(NA)) %>% dplyr::select(all_of(variables))
```
...
men_full_2020_21 <- rbind(men_full_2020   %>% dplyr::select(all_of(variables)), 
                          men_full_2021    %>% dplyr::select(all_of(variables))) %>% 
                    filter(between(date, as.Date("2020-03-02"), as.Date("2021-02-22")))

women_full_2020_21 <- rbind(women_full_2020   %>% dplyr::select(all_of(variables)), 
...
```{r}  
saveRDS    (table1_2020_21_v1, file.path(tables, "table1_2020_21_v1.rds"))
write.xlsx(table1_2020_21_v1, file.path(tables, "table1_2020_21_v1..xlsx"))

saveRDS    (table1_2020_21_v2,   file.path(tables, "table1_2020_21_v2.rds"))
write.xlsx(table1_2020_21_v2, file.path(tables, "table1_2020_21_v2..xlsx"))
