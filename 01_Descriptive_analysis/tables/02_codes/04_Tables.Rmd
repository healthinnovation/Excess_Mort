...
table2_2020_2021        <-      rbind(men_full_2020 %>%   dplyr::select(reg, prov, year, week, sex, group_age, n, E, low, upp), 
                                      ...
                                      women_full_2021 %>% dplyr::select(reg, prov, year, week, sex, group_age, n, E, low, upp)) %>% 
                                group_by(year, group_age, sex) %>% summarise(MoMo_Excess_Deaths_N = round(sum(n) - sum(E)), 
                                                                             COVID_19_Deaths_N = sum(n),
                                                                             E_Deaths_N = round(sum(E))) %>% 
                                                                   bind_rows(summarise(., across(where(is.numeric), sum),
                                                                   ...
                                                                   mutate(Age_Group = case_when(group_age ==  "40_less" ~ "0-40", 
                                                                          ...
                                                                          Ratio_Momo_E_Deaths         = round(MoMo_Excess_Deaths_N/E_Deaths_N,2)) %>% 
                                                                   ungroup %>% 
                                                                   dplyr::select(Period, Sex, Age_Group, MoMo_Excess_Deaths_N, E_Deaths_N, COVID_19_Deaths_N,
                                                                                 Ratio_Momo_E_Deaths, Ratio_Momo_COVID_19_Deaths) %>% 
