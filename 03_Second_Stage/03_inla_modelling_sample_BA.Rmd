...
names(list_excess_m_7.n.17.0) <- list_excess_m_7.n.17.0
...
test_excess_7.n.17.m <- list_excess_m_7.n.17.0 %>% 
...
# 4.2. Exportación
tic()
saveRDS(test_excess_7.n.17.m, file.path(output,"test_excess_6_n_17_0_excess_m_2021.rds")) #m for men / wm for women
toc()
```
## 4.4.1 Análisis
```{r}
summary(test_excess_7.n.17.m[[2]]) #dic (menor valor es el mejor modelo en términos absolutos)
summary(test_excess_7.n.17.m[[2]])$dic$dic
```
...
names(list_excess_w_7.n.8.0) <- list_excess_w_7.n.8.0


# 4.1. Modelado
tic()

test_excess_7.n.8.w <- list_excess_w_7.n.8.0 %>% 
                             purrr::map(~inla.batch.predict.bru.safe(formula = .,
...
# 4.2. Exportación
tic()
saveRDS(test_excess_7.n.8.w, file.path(output,"test_excess_5_n_8_0_excess_w_2021.rds")) #m for men / wm for women
toc()
```
...
## 4.4.1 Análisis
```{r}
summary(test_excess_7.n.8.w[[2]]) #dic (menor valor es el mejor modelo en términos absolutos)
summary(test_excess_7.n.8.w[[2]])$dic$dic
```
