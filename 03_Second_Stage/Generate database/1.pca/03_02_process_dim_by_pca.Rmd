...
social.dets  <- readRDS(file.path(input,"vulnr_df_prov.rds"))         %>% 
                ...
                mutate( educ_dim   = as.numeric(scale.default(pr_edu_inf_25)),
                        demog_dim  = as.numeric(scale.default(pr_no_bl_mz)),
                        econ_dim_3 = as.numeric(scale.default(pr_insu)),
                        econ_dim_4 = as.numeric(scale.default(pr_pov_2018)),
                        econ_dim_5 = as.numeric(scale.default(pr_NBI))
                        )
...
social.dets.dep <- readRDS(file.path(input,"social_dets_dep.rds"))               %>% 
            ...
            dplyr::rename(pr_diab               = Endes_Prop_Diab,
                   ...
                   mutate(med_x_100             = med_x_100/10,  
                        ...
                        health_status_dim_3      = as.numeric(scale.default(pr_diab)),
                        health_status_dim_4      = as.numeric(scale.default(pr_obes)),  
                        health_status_dim_5      = as.numeric(scale.default(pr_tos)),
                        health_performance_dim_3 = as.numeric(scale.default(med_x_100)),
                        health_performance_dim_4 = as.numeric(scale.default(enf_x_100)),
                        health_performance_dim_5 = as.numeric(scale.default(far_x_100))
                        )
...
health.status.pca_1           <- prcomp(social.dets.dep %>% 
                                 select(pr_diab, pr_obes, pr_tos), center = TRUE,scale. = TRUE)

health.status.pca_2           <- prcomp(social.dets.dep %>% 
                                 select(pr_diab, pr_obes, pr_tos, pr_smoke), center = TRUE,scale. = TRUE)

health.performance.pca_1      <- prcomp(social.dets.dep %>% 
                                 select(med_x_100, enf_x_100,
                                        far_x_100), center = TRUE,scale. = TRUE)

health.performance.pca_2      <- prcomp(social.dets.dep %>% 
                                 select(med_x_100, enf_x_100,
...
summary(health.performance.pca_1)
summary(health.performance.pca_2)
summary(health.status.pca_1)
summary(health.status.pca_2)
...
dynamic_pop_dim_1        <-  dynamic.population.pca_1$x %>% 
                             as.data.frame() %>% 
                             select(PC1) %>% 
                             dplyr::rename(dynamic_pop_dim_1  = PC1)

dynamic_pop_dim_2        <-  dynamic.population.pca_2$x %>% 
                             as.data.frame() %>% 
                             select(PC1) %>% 
                             dplyr::rename(dynamic_pop_dim_2  = PC1)

econ_dim_1               <-  economic.pca_1$x %>% 
                             as.data.frame() %>% 
                             select(PC1) %>% 
                             dplyr::rename(econ_dim_1  = PC1)

econ_dim_2               <-  economic.pca_2$x %>% 
                             as.data.frame() %>% 
                             select(PC1) %>% 
                             dplyr::rename(econ_dim_2  = PC1)
...
health_status_dim_1      <-  health.status.pca_1$x %>% 
                             as.data.frame() %>% 
                             select(PC1)  %>% 
                             dplyr::rename(health_status_dim_1 = PC1)

health_status_dim_2      <-  health.status.pca_2$x %>% 
                             as.data.frame() %>% 
                             select(PC1)  %>% 
                             dplyr::rename(health_status_dim_2 = PC1)

health_performance_dim_1  <-  health.performance.pca_1$x %>% 
                              as.data.frame() %>% 
                              select(PC1)  %>% 
                              dplyr::rename(health_performance_dim_1 = PC1)

health_performance_dim_2  <-  health.performance.pca_2$x %>% 
                              as.data.frame() %>% 
                              select(PC1)  %>% 
                              dplyr::rename(health_performance_dim_2 = PC1)



social.dets.pca <- left_join( cbind(social.dets, dynamic_pop_dim_1, dynamic_pop_dim_2, econ_dim_1, econ_dim_2, house_charact_dim_1, house_charact_dim_2) ,
                              cbind(health_status_dim_1,health_status_dim_2, health_performance_dim_1, health_performance_dim_2,
                   ...
                   dplyr::select(dep, prov, 
                                 educ_dim, demog_dim, 
                                 dynamic_pop_dim_1, dynamic_pop_dim_2,
                                 econ_dim_1, econ_dim_2, econ_dim_3, econ_dim_4, econ_dim_5,
                                 house_charact_dim_1, house_charact_dim_2,
                                 health_status_dim_1, health_status_dim_2, health_status_dim_3, health_status_dim_4, health_status_dim_5,
                                 health_performance_dim_1, health_performance_dim_2, health_performance_dim_3, health_performance_dim_4, health_performance_dim_5)
